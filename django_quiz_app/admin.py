...
class UserResponseAdmin(admin.ModelAdmin):
    list_display = ('user', 'question',
                    'selected_choice', 'score', 'quiz_title', 'quiz_total_score')
    ...
    def quiz_total_score(self, obj):
        user_responses = UserResponse.objects.filter(
            user=obj.user, quiz=obj.quiz)
        total = user_responses.aggregate(total_score=sum(
            'selected_choice__score'))["total_score"]
